...
fnmCSV = '/zdsk/Manuscripts/Baydoun/UK-BioBank/2024-02-13--13--ProteomicCVD/Volcano/LE8_PROTEOME_DMRIVOLCANOPLOTDATA.csv'
...
Volcano = function(volDat) {
	ggObj =	ggplot(data=volDat, aes(x=estimate, y=mlog10p, label=lblOutcome)) +
		...
		geom_point(shape=20, size=3, col='orange', data=subset(volDat, selected_final==1 & signif==1 & estimate>  0.25)) +
                geom_point(shape=20, size=3, col='blue',   data=subset(volDat, selected_final==1 & signif==1 & estimate< -0.20)) +
		xlim(-.45, .45) + ylim(0, 250) +
		geom_text_repel(size=3, col='red', data=subset(volDat,  selected_final==1 & estimate>  0.25), aes(label=lblOutcome)) +
...
dirName = '/zdsk/Manuscripts/Baydoun/UK-BioBank/2024-02-13--13--ProteomicCVD/Volcano/'
imgName = '2024-02-24-UKB13-Volcano'
